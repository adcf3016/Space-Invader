...
data segment
	plane_pos WORD 150,180		;飞机位置存储
	smile_pos BYTE 10,3,12,4,15,2,19,4,21,1,25,5,28,3,30,1,9,2,7,5,1,2,33,3,35,2,0,0		;笑脸位置存储(y,x)
	destroy	  BYTE 0			;是否有目标
	delay_timer BYTE 0
	timecontrol BYTE 18		;用于控制速度快慢
	message BYTE 'Your score:','0'
	score BYTE 3 dup('0'),'0';分数的ASCII码，用于显示
    	score_b BYTE 00h;分数的二进制码，用于运算
	;用于画游戏开始菜单
    	message_welcome BYTE '*************** welcome ***************','0'
    	message_operation BYTE "how to play:",'0'
    	message_operation1 BYTE "move:left right up and down",'0'
    	message_operation2 BYTE 'shoot:space bar','$'
		message_operation3 BYTE 'score: hit(+1) escape(-5) collide(GameOver)','0'
    	message_operation4 BYTE 'Now,you can:','0'
    	start_button BYTE "press 'Enter' to start the game0"
    	end_button BYTE "press 'Esc' to quit(also in the game).0"
    	message_end BYTE '********13349152 Zhang Huajian*********','0'
	message_easy BYTE "1.easy",'$'
	message_mid BYTE "2.middle",'$'
	message_hard BYTE "3.hard",'$'
	message_veryhard BYTE "4.veryhard",'$'
	message_choose BYTE "please choose:",'$'
	    message_over1 BYTE '****************************','$'
		message_over2 BYTE '*******   GAME OVER  *******','$'
    	message_over3 BYTE '****************************','$'
data ends
...
start:
	...
	xor ax,ax			; AX = 0
	mov ds,ax			; DS = 0
	mov DWORD ptr ds:[20h],offset Timer	; 设置时钟中断向量的偏移地址
	mov ax,cs 
...
play_plane_1 BYTE 6,1,1,5,2,3,5,3,3,5,4,3,4,5,5,3,6,7,1,7,11,1,8,11,4,9,5,5,10,3,4,11,5,3,12,7,4,13,2,7,13,2 ;X0,Y,长度
...
play_smile proc
	...
	mov esi,offset smile_pos
	inc esi
	mov edi,offset smile_pos
show_smile:	
	;设置光标位置
	mov ah,02H
	mov bh,0
...
help_view proc  ;显示开始菜单
	 ...
	 int 10h
	 mov ah,09
	 mov edx,offset message_welcome
	 int 21h	 
	 mov ah,02
	 mov bh,00
	 mov dh,06
	 ...
	 int 10h
	 mov ah,09
	 mov edx,offset message_operation
	 int 21h	 
	 mov ah,02
	 mov bh,00
	 mov dh,08
	 ...
	 int 10h
	 mov ah,09
	 mov edx,offset message_operation1
	 int 21h	 
	  mov ah,02
	 mov bh,00
	  mov dh,10
	 ...
	 int 10h
	 mov ah,09
	 mov edx,offset message_operation2
	 int 21h	 
	  mov ah,02
	 mov bh,00
	  mov dh,12
	 ...
	 int 10h
	 mov ah,09
	 mov edx,offset message_operation3
	 int 21h
	  mov ah,02
	 mov bh,00
	  mov dh,14
	 ...
	 int 10h
	 mov ah,09
	 mov edx,offset message_operation4
	 int 21h
	  mov ah,02
	 mov bh,00
	  mov dh,16
...
choose_view proc  ;显示难度选择菜单
	 ...
	 int 10h
	 mov ah,09
	 mov edx,offset message_veryhard
	 int 21h
	  mov ah,02
	 mov bh,00
	  mov dh,12
